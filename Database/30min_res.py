"""
30-Minute Resolution Dataset Exporter  ── v3
=============================================
Fix log vs v2:
...
# ──────────────────────────────────────────────
INTERVAL_MINS = 30
FUTURE_STEPS  = int((24 * 60) / INTERVAL_MINS)   # 48
...
-- ── 2. PER-ROOM START — SNAPPED TO 30-MIN GRID ───────────────────────────────
-- FIX-8: min_ts is the real first record time (e.g. 10:07:33).
-- Snap it DOWN to the nearest 30-min boundary (→ 10:00) so every bucket
-- in generate_series lands on :00 or :30.
-- Formula:  date_trunc('hour', min_ts)
--         + floor(minute(min_ts) / interval_mins) * interval
bounds AS (
    SELECT
        rs.room,
        ...
        date_trunc('hour', MIN(rs.last_update AT TIME ZONE 'America/New_York'))
        + FLOOR(
            EXTRACT(MINUTE FROM MIN(rs.last_update AT TIME ZONE 'America/New_York'))
            / {interval_mins}
          ) * interval {interval_str}
        AS min_ts
    FROM room_state rs
...
def post_process(df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"  Future cols      : {'✓  all 48 present' if not missing else f'⚠  {len(missing)} missing'}")
